...
void ir_rx_init()
{
	...
	rmt_rx.rx_config.filter_ticks_thresh = RMT_CLK_DIV;
	rmt_rx.rx_config.idle_threshold = 0xffffffff;
	rmt_config(&rmt_rx);
...
void ir_send_raw(const unsigned int buff[], unsigned int len, unsigned int freq)
{
	...
	size_t size = (sizeof(rmt_item32_t) * len);
	...
	rmt_write_items(txChannel, item, len, true);
...
void ir_send_codes_from_string(char *irCodes)
{
	...
	{
		ir_send_raw(irTimings,index,38000);
	}
}
...
*/
			buff[i*2] = (item->duration0*10)/RMT_TICK_10_US;
			buff[i*2+1] = (item->duration1*10)/RMT_TICK_10_US;		
			
			#if RMT_DEBUG_ENABLE
			printf( "\n [%2d]item->duration0:%d item->duration1:%d  [%d-%d]", i,item->duration0,item->duration1, buff[i*2], buff[i*2+1]);
			#endif  
